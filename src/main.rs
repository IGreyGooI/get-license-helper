...
fn get_license(
    ...
    if !success {
        ...
        let license_names: Vec<String> = license_names
            .iter()
            .map(|l| vec![l.to_string(), format!("{}.txt", l), format!("{}.md", l)])
            .flatten()
            .collect();
                    ...
                    match result {
                        Ok(mut resp) => {
                            if resp.status().is_success() {
                                ...
                                success = true;
                                break 'outer;
                            } else if resp.status().eq(&reqwest::StatusCode::NOT_FOUND){
                                break 'retry;
                                //println!("Response is invalid, status: {}", resp.status())
                            }
                        }
                        Err(err) => {
                            //println!("Error when sending the request to {}, err: {}, ", url, err)
                        }
                    }
